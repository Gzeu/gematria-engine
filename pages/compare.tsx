...
function RadarViz({ dataA, dataB, wordA, wordB, ciphers }: {
  ...
  const pt = (i: number, val: number) => {
    const sc = (val / max) * r;
    return `${cx + sc * Math.cos(ang(i))},${cy + sc * Math.sin(ang(i))}`;
  };
  ...
  const shortLabel = (name: string) =>
    name.replace('English Ordinal', 'Ord').replace('Eng. Reduction', 'Red')
      ...
      .replace('English Sumerian', 'Sum').replace('Chaldean', 'Chl');
  return (
    <div className={s.radarWrap}>
      <svg viewBox="0 0 300 300" className={s.radarSvg} aria-hidden>
        <defs>
          ...
          <radialGradient id="rgB" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#3db8b4" stopOpacity="0.28" />
            <stop offset="100%" stopColor="#3db8b4" stopOpacity="0.04" />
          </radialGradient>
        </defs>
        {[0.25, 0.5, 0.75, 1].map(sc => (
          <polygon key={sc} points={ringPts(sc)} fill="none" stroke="rgba(255,245,220,0.06)" strokeWidth="1" />
        ))}
        {ciphers.map((_, i) => (
          <line key={i} x1={cx} y1={cy}
            x2={cx + r * Math.cos(ang(i))} y2={cy + r * Math.sin(ang(i))}
            stroke="rgba(255,245,220,0.05)" strokeWidth="1" />
        ))}
        <polygon points={polyB} fill="url(#rgB)" stroke="#3db8b4" strokeWidth="1.5" strokeOpacity="0.85" />
        <polygon points={polyA} fill="url(#rgA)" stroke="#a78bfa" strokeWidth="1.5" strokeOpacity="0.9" />
        {ciphers.map((c, i) => {
          const [ax, ay] = pt(i, dataA[c.key] ?? 0).split(',').map(Number);
          const [bx, by] = pt(i, dataB[c.key] ?? 0).split(',').map(Number);
          return (
            <g key={c.key}>
              <circle cx={ax} cy={ay} r={3} fill="#a78bfa" />
              <circle cx={bx} cy={by} r={3} fill="#3db8b4" />
            </g>
          );
        })}
        {ciphers.map((c, i) => {
          ...
          return (
            <text key={c.key} x={lx} y={ly}
              ...
              fontSize="8.5" fontFamily="var(--fm)" fill="rgba(255,245,220,0.4)"
      ...
      <div className={s.radarLegend}>
        <div className={s.radarLi}>
          <span className={s.radarDot} style={{ background: '#a78bfa' }} />
          <span style={{ color: '#c4b5fd', fontWeight: 600 }}>{wordA}</span>
        </div>
        <div className={s.radarLi}>
          <span className={s.radarDot} style={{ background: '#3db8b4' }} />
          <span style={{ color: '#5ee8e4', fontWeight: 600 }}>{wordB}</span>
        </div>
      </div>
    </div>
  );
...
function BarsViz({ dataA, dataB, wordA, wordB, ciphers }: {
  ...
  const max = Math.max(...ciphers.flatMap(c => [dataA[c.key] ?? 0, dataB[c.key] ?? 0]), 1);
  const short = (n: string) => n.replace('English Ordinal', 'Ordinal').replace('Eng. Reduction', 'Reduction')
    .replace('Reverse Ordinal', 'Reverse').replace('Hebrew Standard', 'Hebrew').replace('English Sumerian', 'Sumerian');
  return (
    <div className={s.barsWrap}>
      <div className={s.barsLegend}>
        <span className={s.barsLegDot} style={{ background: '#a78bfa' }} />
        <span style={{ color: '#c4b5fd' }}>{wordA}</span>
        <span className={s.barsLegDot} style={{ background: '#3db8b4', marginLeft: 'var(--sp5)' }} />
        <span style={{ color: '#5ee8e4' }}>{wordB}</span>
      </div>
      ...
      {ciphers.map(c => {
        ...
        return (
          <div key={c.key} className={s.dotsRow}>
            <div className={s.dotsLabel}>{c.name}</div>
            <div className={s.dotsCols}>
              <div className={s.dotsSide}>
                <div className={s.dotsWord} style={{ color: '#c4b5fd' }}>
                  {wordA} <em className={s.dotsNum}>{dataA[c.key]}</em>
                </div>
                <div className={s.dotGrid}>
                  {Array.from({ length: MAX_DOTS }).map((_, i) => (
                    <div key={i} className={s.dot}
                      style={{ background: i < av ? '#a78bfa' : 'rgba(255,245,220,0.06)' }} />
                  ))}
                </div>
              </div>
              <div className={s.dotsSide}>
                <div className={s.dotsWord} style={{ color: '#5ee8e4' }}>
                  {wordB} <em className={s.dotsNum}>{dataB[c.key]}</em>
                </div>
                <div className={s.dotGrid}>
                  {Array.from({ length: MAX_DOTS }).map((_, i) => (
                    <div key={i} className={s.dot}
                      style={{ background: i < bv ? '#3db8b4' : 'rgba(255,245,220,0.06)' }} />
                  ))}
                </div>
              </div>
            </div>
...
export default function Compare() {
  ...
  function scheduleAnalyze(wA: string, wB: string) {
    ...
    if (wA.trim().length >= 2 && wB.trim().length >= 2)
      debounceRef.current = setTimeout(() => runAnalyze(wA, wB), 500);
  }
  ...
  const simGlow = simScore >= 80 ? '#3db8b4' : simScore >= 50 ? '#e8af34' : '#d163a7';
          ...
          <section className={s.hero}>
            <div className={s.inputCard} data-accent="a">
              <div className={s.inputLabel}>
                <span className={s.inputPip} style={{ background: '#a78bfa' }} />
                Word A
              </div>
              <input
            ...
            <div className={s.inputCard} data-accent="b">
              <div className={s.inputLabel}>
                <span className={s.inputPip} style={{ background: '#3db8b4' }} />
                Word B
              </div>
              <input
                className={s.wordInput}
            ...
            <div className={s.results}>
              ...
              <div className={s.bento}>
                ...
                <div className={s.bentoCard} data-match={String(matchCount === CIPHER_DEFS.length)}>
                  ...
                  {matchCount === CIPHER_DEFS.length && (
                    <div className={s.bentoMatchFull}>✦ PERFECT ✦</div>
                  )}
                </div>
                ...
                <div className={s.bentoSim} style={{ '--glow': simGlow } as React.CSSProperties}>
                  <svg viewBox="0 0 120 70" className={s.gaugesvg} aria-hidden>
                    <path d="M 15 65 A 55 55 0 0 1 105 65" fill="none" stroke="rgba(255,245,220,0.08)" strokeWidth="9" strokeLinecap="round" />
                    <path d="M 15 65 A 55 55 0 0 1 105 65" fill="none" stroke={simGlow}
                      strokeWidth="9" strokeLinecap="round" strokeOpacity="0.9"
                      strokeDasharray={`${(simScore / 100) * 172.8} 172.8`}
                      style={{ filter: `drop-shadow(0 0 5px ${simGlow}88)` }}
                    />
                    <text x="60" y="62" textAnchor="middle" fontSize="20" fontWeight="700"
                      fontFamily="var(--fm)" fill="var(--tx)">{simScore}%</text>
                  </svg>
                ...
                <div className={s.bentoNum}>
                  ...
                  <div className={s.bentoNumRow}>
                    <span className={s.bentoNumLabel} style={{ color: '#5ee8e4' }}>{dataB!.original_input}</span>
                    <span className={s.bentoNumVal} style={{ color: '#5ee8e4' }}>{dataB!.base_number}</span>
                  </div>
              ...
              <div className={s.table}>
                <div className={s.tableHead}>
                  ...
                  <span className={s.thVal} style={{ color: '#5ee8e4' }}>{dataB!.original_input}</span>
                  <span />
                </div>
                    ...
                    <div key={d.key} className={s.tableGroup}>
                      ...
                      >
                        ...
                        <div className={s.simBadge}>
                          <span className={s.simPct} style={{ color: sim === 100 ? '#3db8b4' : sim >= 70 ? '#e8af34' : 'var(--txm)' }}>
                            {sim}%
                          </span>
                        </div>

                        <div className={s.deltaBadge} style={{
                          color: isMatch ? '#3db8b4' : diff > 0 ? '#c4b5fd' : '#5ee8e4'
                        }}>
                          {isMatch ? '=' : diff > 0 ? `+${diff}` : String(diff)}
                        </div>

                        <div className={s.valGroup} style={{ alignItems: 'flex-end' }}>
                          <span className={`${s.valNum} ${bWins ? s.valWinner : ''}`} style={{ color: '#5ee8e4' }}>{bv}</span>
                          <div className={s.miniBar}>
                      ...
                      {open && (
                        <div className={s.breakdown}>
                          ...
                          <div className={s.bdCol}>
                            <div className={s.bdTitle} style={{ color: '#5ee8e4' }}>
          ...
          {!hasData && !loading && (
            <div className={s.empty}>
              ...
              <p className={s.emptyText}>
                Enter any two words to reveal their gematric relationship across all 6 cipher systems.
              </p>
              <div className={s.emptyHints}>
                <span className={s.emptyHint}>
                  Try:{' '}
                  <button className={s.emptyLink} onClick={() => { setInputA('fire'); setInputB('water'); scheduleAnalyze('fire', 'water'); }}>
                    fire vs water
                  </button>
                </span>
                <span className={s.emptyHint}>
                  or{' '}
                  <button className={s.emptyLink} onClick={() => { setInputA('sun'); setInputB('god'); scheduleAnalyze('sun', 'god'); }}>
                    sun vs god
                  </button>
                </span>
              </div>
